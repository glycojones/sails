...
int main( int argc, char** argv )
{
    ...
    {
        float map_value = min_map.interp<clipper::Interp_cubic>(min_map.coord_map(mol[0][0][natm-1].coord_orth()));
     //   std::cout << "map_value: " << map_value << std::endl;
      //  std::cout << "mean_minmap: " << mean_minmap << std::endl;
      //  std::cout << "stdev_minmap: " << stdev_minmap << std::endl;

        if (map_value  > mean_minmap - (stdev_minmap))
        {
            ...
            std::cout << "{ \"" << mol[0][0][natm-1].id().trim() << "\", " << mol[0][0][natm-1].coord_orth().x() << ", "
