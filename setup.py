...
def run_sails_install_script():
    build_sails_cmd = ['bash', 'DependencyInstaller.sh']
    if subprocess.call(build_sails_cmd) != 0:
        sys.exit("Failed to build Sails's dependencies")

# I make a new command that will build the shared-library
class build_sailsdeps(Command):
    user_options = []
...
setup(
    ...
    package_dir={'':'source'},
    ext_modules=[CMakeExtension('sails/sails')],
    cmdclass={
        'install': install,
        'build_sailsdeps': build_sailsdeps, 'build': build,
        'develop': develop,
