...
mainDir=$PWD
dependencyDir=$mainDir/dependencies/privateer/dependencies
...
# cd $dependencyDir
# if [[ ! -d include/gsl ]]; then
# echo "gsl installation ... falied. We can not continue the rest of the installation steps."
# exit 3
# fi
...
# if [[  -d include/gsl ]]; then
# echo "gsl is installed. Remove include/gsl and gsl folder  if you want to re-install"
# fi
...
# if [[ -f lib/libgsl.so  ]]; then
# echo "gsl "
# readelf -p .comment lib/libgsl.so | grep  "GCC" | sort --unique
# fi
fi
...
# if [[ -f $dependencyDir/lib/libgsl.so ]]; then
# GSL=$dependencyDir/lib/libgsl.so
# fi
